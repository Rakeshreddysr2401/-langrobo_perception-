...
#
# UNITS. rover_firmware_v2.ino reads wz as RADIANS PER SECOND and computes each
# wheel as  vx +/- wz * WHEEL_BASE_M/2  with WHEEL_BASE_M = 0.34. It does NOT
# treat wz as a PWM fraction -- that was an earlier firmware (rover_sim
# contract_bridge.py), and the values below were tuned for it.
#
# The consequence, measured 2026-08-22: WZ = 2.0 asks each wheel for 0.34 m/s,
# which against a 0.86 m/s maximum is ~47% duty -- not the ~100% the old comment
# claimed. Lifted on blocks the wheels counter-rotate cleanly at that duty; on
# the floor they cannot break four tyres loose sideways, so a pivot command
# turned into the rover driving backward or forward with a slight curve. The
# operator drove 10.7 m of "room loop" inside a 1.8 m box because of it.
#
# Full authority is wz = 2 * 0.86 / 0.34 = 5.06 rad/s, where both wheels reach
# maximum speed in opposite directions. WZ is set just under that.
VX = 0.20
WZ = 5.0           # pivot: both wheels ~full duty, opposite directions
# Tight forward-curving turn: inner wheel drives backward, outer forward, with a
# little net forward creep. Scaled by the same 2.5x as WZ, for the same reason.
VX_SLIGHT = 0.08
WZ_SLIGHT = 4.25
MOVES = {
    'forward': (VX, 0.0),
