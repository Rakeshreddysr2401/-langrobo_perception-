...
| **2026-08-11** | **The laptop desktop is Wayland, where `loginctl` reports an empty Display.** `run_stack.sh view start` guessed `:0` and passed no X auth cookie, so rviz2 died in under a second. | `:0` was right; the cookie was missing. Both now read from the Xwayland cmdline (`-auth /run/user/1000/.mutter-Xwaylandauth.*`). The error only ever appeared in `/tmp/rviz.log` **on the laptop**. |
| **2026-08-11** | **The raw camera image display starves the map over wifi.** Enabling `/camera/camera0/color/image_raw` (uncompressed, ~30 Hz) in RViz dropped the nvblox occupancy grid arriving at the laptop from **9.4 Hz to 0**, and stretched `/odom` gaps to 0.47 s. | Disabling it restored 9.3 Hz immediately. Keep the laptop view light. Same failure family as the 3D point-cloud displays. |
| **2026-08-11** | **There is no URDF in this repo** — RViz had nothing to draw, so the rover appeared only as bare TF axes. | Added `nodes/rover_marker.py`: chassis box at the measured 30×17 cm, forward arrow, camera at (0.10, 0.163), and the ~87° FOV wedge. Started by `run_stack.sh up`. |
| **2026-08-11** | **The map was being deleted behind the rover — it could never be more than a torch beam.** Two mechanisms in nvblox's base config: (1) TSDF decay, `decay_tsdf_rate_hz 5.0` × `tsdf_decay_factor 0.95` with `deallocate_decayed_blocks: true` — from max weight 5.0 to the 0.001 threshold is `ln(0.001/5)/ln(0.95)` = 166 steps = **~33 s**, after which the block is freed; (2) `map_clearing_radius_m: 7.0` at 1 Hz deleting everything beyond 7 m. | Those defaults suit NVIDIA's dynamic-scene demos, where stale geometry *should* be forgotten. Ours is the opposite goal. Fixed in `config/nvblox.yaml`: decay factor 0.999999 (~20 days), no deallocation, clearing radius −1.0, and visualization/streamer radii raised from 7 m to 100 m. **Cost: the map now never forgets** — moving objects leave permanent ghosts and drift smears walls. Use `remap` for a clean origin. |
| **2026-08-11** | `tsdf_decay_factor: 1.0` **hard-aborts nvblox.** `tsdf_decay_integrator.cu:27` asserts `value < 1.0`. | The node dies at startup with `Check failed: value < 1.0 (1 vs. 1)` and never comes back. Must be strictly less than 1. |
