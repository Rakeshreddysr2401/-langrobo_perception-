...
|---|---|---|---|
| 2026-08-11 | 00 | **Every nvblox restart cycles the D555's depth stream — so `fuse` is not free.** Strong suspicion, not yet proven. Ran `up`/`fuse`/`cam` about six times in one hour; the camera degraded from 26 Hz to 9.8 Hz (`trust=False status=CAMERA_STARVED`) at `load1` of only 3.1, so the Orin was *not* the bottleneck, then went fully offline. Mechanism is the same as the probe trap below: nvblox is the only depth subscriber, so killing it drops the subscriber count to zero and the driver stops the stream over the fragile DDS control channel. Evidence: killing `nvblox_node` alone left depth at 0 Hz permanently, with nvblox logging "Last view not set for sensor type". **Bring the stack up once, then drive — do not re-run `fuse` casually.** | **known, minimise restarts** |
...
| 2026-08-11 | 01/02 | **Over-thick map traced to the pose, not to nvblox.** A *stationary* rover builds a healthy map (occ/free 0.46, stable) with zero pose drift. Ruled out by measurement: the floor entering the slice band (ratio unchanged at 0.47 vs 0.48 across two bands), noise accumulation (only +5 cells/min), and parked pose drift (0.0000 m over 160 s). The 1.66 blob only appears after **driving** — so the remaining suspect is pose error in motion. Next test is issue 02's: push a measured 2.00 m and compare `/odometry/filtered`. | **open — do issues 01/02 first** |
