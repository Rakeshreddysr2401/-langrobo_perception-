"""3D object detections for the brain — YOLO + D555 depth → odom-frame JSON.

The Pi5 brain's approach_object tool ("come here", "go near the chair") needs
metric object positions in a frame it trusts. This node runs YOLOv8n on the D555
color stream, samples the (unaligned) depth image at each detection via
intrinsics-angle mapping, deprojects to a 3D point in the depth optical frame,
transforms it with TF, and publishes the Pi5 contract:

  /vision/detections_3d   std_msgs/String   JSON:
    {"frame": "odom", "stamp": 1752300000.0,
     "objects": [{"label": "chair", "x": 2.31, "y": -0.42, "z": 0.35,
                  "conf": 0.81}, ...]}

Contract rules (ros2_bridge.on_detections on the Pi5):
  - frame MUST be "odom" ON THIS STACK, and we publish NOTHING until TF
    odom→optical resolves. This node was written against the older stack, whose
    contract was `map`; the brain now DROPS (and logs) any frame that is not
    `odom` — see TODO 32. `target_frame` is a parameter, so the frame moves when
    the stack does; the default is what the brain currently accepts.
...
It CAN also republish the color stream as JPEG on
/camera/color/image_raw/compressed, which is what fed the brain's look() tool on
the older stack. **That is off by default here** (`look_feed_rate:=0.0`): on this
stack `phase4/nodes/image_bridge.py` owns that topic, and two publishers on one
topic is the hazard that costs you an afternoon. Turn it on only if you are
running this node INSTEAD of the `vlm` layer, not alongside it.
...
Design notes:
  ...
  - Runs in the `rover` container (orin-nav:1.1), which already carries
    torch 2.12.0+cu130 and ultralytics 8.4.80 with CUDA available. The model is
    mounted read-only at /models — see `./rover detect`.
"""
...
class Detections3DNode(Node):
    def __init__(self):
        ...
        # "odom", not "map": the Pi5 brain drops any other frame (TODO 32), and
        # on this stack nav2 plans in odom within a single session anyway.
        ...
        # /camera/color/image_raw/compressed. DISABLED by default on this stack:
        # image_bridge (the `vlm` layer) publishes that topic, and a second
        # publisher on it would be indistinguishable from a flapping camera.
        self.declare_parameter("look_feed_rate", 0.0)
