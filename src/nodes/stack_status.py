...
WHY THIS EXISTS
    ...
    ever had was a RATE COLLAPSE, not an absence:
      ...
      * ESP32 /wheel_state at 1 Hz instead of 20 Hz (publisher count: still 1)
        — since reflashed; it now runs at 10 Hz, still half the firmware's 20 (TODO.md)
...
# fault -- so absence is reported as "not started (run: ./rover.sh lN)" rather than as a
# problem. A topic whose owner IS running but whose rate has collapsed is the real
# failure mode this tool exists to catch.
WATCH = [
    ("/camera/camera0/infra1/image_rect_raw", "camera IR left",  15.0, "l1",
     "cuVSLAM input. <10 Hz => cuVSLAM freezes SILENTLY and never recovers"),
    ("/camera/camera0/depth/image_rect_raw",  "camera depth",    10.0, "l1",
     "nvblox + safety_guard bumper input"),
    ("/odom",                                 "cuVSLAM odom",    10.0, "l2",
     "visual odometry. 0 Hz with the camera alive = cuVSLAM FROZEN, restart it"),
    ("/nvblox_node/static_map_slice",         "nvblox slice",     5.0, "l4",
     "the costmap's obstacle layer"),
    ("/odometry/filtered",                    "EKF odom",        10.0, "l3",
     "fused pose"),
    ("/imu/base",                             "IMU (gyro)",      50.0, "l3",
     "gyro yaw into the EKF"),
    ("/perception/depth_points",              "collision src",    5.0, "l5",
    ...
    ("/wheel_state",                          "ESP32 encoders",  15.0, "esp32",
     ...
     "the connected loop at all. Diagnose before reflashing (TODO.md)"),
...
def main():
    ...
    for topic, label, want, owner, why in WATCH:
        ...
        if not node.present[topic]:
            mark, note = "  --", f"not started (run: ./rover.sh {owner})"
            if want > 0:
