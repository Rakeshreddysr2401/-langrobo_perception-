...
DRIFT. Until robot_state_publisher takes over the TFs (ROVER_BUILD_PLAN.md step
4), the same numbers are still typed into ./rover, nav2.yaml, rover_marker.py,
the firmware and the Phase 1 nodes. The check reads each copy and fails if it
disagrees with what this derives, so a new measurement cannot land in one place
and not the others.
"""
import math
import re
...
REPO = HERE.parent
TOL = 1e-4
STATUSES = ('measured', 'datasheet', 'calibrated', 'described', 'unknown')
...
def drift(g):
    ...
    cam = grab('rover', rf'static_transform_publisher --x {num} --y {num} --z {num} \\\s*\n'
                        r'\s*--frame-id base_link --child-frame-id camera0_link', 3)
    checks = [
        ('./rover LIDAR_X', grab('rover', r'^LIDAR_X=\$\{LIDAR_X:-([-\d.]+)\}'), g['lidar'][0]),
        ('./rover LIDAR_Y', grab('rover', r'^LIDAR_Y=\$\{LIDAR_Y:-([-\d.]+)\}'), g['lidar'][1]),
        ('./rover LIDAR_Z', grab('rover', r'^LIDAR_Z=\$\{LIDAR_Z:-([-\d.]+)\}'), g['lidar'][2]),
        ('./rover LIDAR_YAW', grab('rover', r'^LIDAR_YAW=\$\{LIDAR_YAW:-([-\d.]+)\}'), g['lidar_yaw']),
        ('./rover camera TF x,y,z', cam, g['cam']),
