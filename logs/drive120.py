...
# REFUSE TO DRIVE IF ANYTHING ELSE OWNS /cmd_vel.
#
# The teleop streams zeros at 10 Hz while it is in MANUAL. Those zeros
# interleave with nav2's commands and cancel them, so the rover crawls, stalls,
# and eventually sits still while nav2 believes it is driving. Measured
# 2026-08-22: a 120 cm goal stalled at 101 cm and sat there for 50 seconds
# commanding nothing, because every nav2 command was being overwritten.
#
# An earlier version of this script PRINTED a warning and drove anyway, which
# produced exactly that run and wasted it. A warning you drive through is not a
# check.
print("  checking nothing else owns /cmd_vel ...")
watch_until = time.time() + 3.0
cmds.clear()
while time.time() < watch_until:
    rclpy.spin_once(n, timeout_sec=0.1)
if cmds:
    print("  ABORT: /cmd_vel is being published by something else (%d msgs in 3 s)."
          % len(cmds))
